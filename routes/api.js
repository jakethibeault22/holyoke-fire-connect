...
router.post('/admin/init-supabase', async (req, res) => {
  ...
  try {
    ...
    const password_hash = crypto.createHash('sha256').update('SuperAdmin123!').digest('hex');
    ...
    const userResult = await client.query(
      `INSERT INTO users (email, name, username, password_hash, is_admin, role, status) 
       ...
       RETURNING id`,
      ['superadmin@holyokefire.com', 'Super Administrator', 'superadmin', password_hash, 1, 'super_user', 'active']
    );
    
    if (userResult.rows.length > 0) {
      await client.query(
        'INSERT INTO user_roles (user_id, role) VALUES ($1, $2) ON CONFLICT DO NOTHING',
        [userResult.rows[0].id, 'super_user']
      );
    }
